const tokens = {
  "colors.sunbeam.orange": {
    "value": "#fa520f",
    "variable": "var(--colors-sunbeam\\.orange)"
  },
  "colors.sunbeam.flame": {
    "value": "#fb6424",
    "variable": "var(--colors-sunbeam\\.flame)"
  },
  "colors.beam.orange": {
    "value": "#ff8105",
    "variable": "var(--colors-beam\\.orange)"
  },
  "colors.sunshine.900": {
    "value": "#ff8a00",
    "variable": "var(--colors-sunshine\\.900)"
  },
  "colors.sunshine.700": {
    "value": "#ffa110",
    "variable": "var(--colors-sunshine\\.700)"
  },
  "colors.sunshine.500": {
    "value": "#ffb83e",
    "variable": "var(--colors-sunshine\\.500)"
  },
  "colors.sunshine.300": {
    "value": "#ffd06a",
    "variable": "var(--colors-sunshine\\.300)"
  },
  "colors.sunshine.50": {
    "value": "rgba(255,161,16,0.50)",
    "variable": "var(--colors-sunshine\\.50)"
  },
  "colors.sunshine.35": {
    "value": "rgba(255,161,16,0.35)",
    "variable": "var(--colors-sunshine\\.35)"
  },
  "colors.sunshine.25": {
    "value": "rgba(255,161,16,0.25)",
    "variable": "var(--colors-sunshine\\.25)"
  },
  "colors.beam.gold": {
    "value": "#ffe295",
    "variable": "var(--colors-beam\\.gold)"
  },
  "colors.bright.yellow": {
    "value": "#ffd900",
    "variable": "var(--colors-bright\\.yellow)"
  },
  "colors.warm.ivory": {
    "value": "#fffaeb",
    "variable": "var(--colors-warm\\.ivory)"
  },
  "colors.cream": {
    "value": "#fff0c2",
    "variable": "var(--colors-cream)"
  },
  "colors.sunbeam.black": {
    "value": "#1f1f1f",
    "variable": "var(--colors-sunbeam\\.black)"
  },
  "colors.card.dark": {
    "value": "#2a2a2a",
    "variable": "var(--colors-card\\.dark)"
  },
  "colors.code.activePill": {
    "value": "#404040",
    "variable": "var(--colors-code\\.active-pill)"
  },
  "colors.code.text": {
    "value": "#d4d4d8",
    "variable": "var(--colors-code\\.text)"
  },
  "colors.code.success": {
    "value": "#4ade80",
    "variable": "var(--colors-code\\.success)"
  },
  "colors.syn.keyword": {
    "value": "#c084fc",
    "variable": "var(--colors-syn\\.keyword)"
  },
  "colors.syn.fn": {
    "value": "#93c5fd",
    "variable": "var(--colors-syn\\.fn)"
  },
  "colors.syn.string": {
    "value": "#86efac",
    "variable": "var(--colors-syn\\.string)"
  },
  "colors.syn.prop": {
    "value": "#fdba74",
    "variable": "var(--colors-syn\\.prop)"
  },
  "colors.syn.number": {
    "value": "#fb923c",
    "variable": "var(--colors-syn\\.number)"
  },
  "colors.syn.builtin": {
    "value": "#fde047",
    "variable": "var(--colors-syn\\.builtin)"
  },
  "colors.border.warm": {
    "value": "rgba(127, 99, 21, 0.15)",
    "variable": "var(--colors-border\\.warm)"
  },
  "colors.border.warmSubtle": {
    "value": "rgba(127, 99, 21, 0.08)",
    "variable": "var(--colors-border\\.warm-subtle)"
  },
  "colors.border.warmDark": {
    "value": "rgba(255, 161, 16, 0.15)",
    "variable": "var(--colors-border\\.warm-dark)"
  },
  "colors.accent.06": {
    "value": "rgba(250,82,15,0.06)",
    "variable": "var(--colors-accent\\.06)"
  },
  "colors.accent.08": {
    "value": "rgba(250,82,15,0.08)",
    "variable": "var(--colors-accent\\.08)"
  },
  "colors.accent.10": {
    "value": "rgba(250,82,15,0.10)",
    "variable": "var(--colors-accent\\.10)"
  },
  "colors.accent.12": {
    "value": "rgba(250,82,15,0.12)",
    "variable": "var(--colors-accent\\.12)"
  },
  "colors.accent.15": {
    "value": "rgba(250,82,15,0.15)",
    "variable": "var(--colors-accent\\.15)"
  },
  "colors.accent.20": {
    "value": "rgba(250,82,15,0.20)",
    "variable": "var(--colors-accent\\.20)"
  },
  "colors.accent.30": {
    "value": "rgba(250,82,15,0.30)",
    "variable": "var(--colors-accent\\.30)"
  },
  "colors.accent.40": {
    "value": "rgba(250,82,15,0.40)",
    "variable": "var(--colors-accent\\.40)"
  },
  "colors.chrome.03": {
    "value": "rgba(255,255,255,0.03)",
    "variable": "var(--colors-chrome\\.03)"
  },
  "colors.chrome.05": {
    "value": "rgba(255,255,255,0.05)",
    "variable": "var(--colors-chrome\\.05)"
  },
  "colors.chrome.06": {
    "value": "rgba(255,255,255,0.06)",
    "variable": "var(--colors-chrome\\.06)"
  },
  "colors.chrome.10": {
    "value": "rgba(255,255,255,0.10)",
    "variable": "var(--colors-chrome\\.10)"
  },
  "colors.chrome.30": {
    "value": "rgba(255,255,255,0.30)",
    "variable": "var(--colors-chrome\\.30)"
  },
  "colors.chrome.35": {
    "value": "rgba(255,255,255,0.35)",
    "variable": "var(--colors-chrome\\.35)"
  },
  "colors.chrome.40": {
    "value": "rgba(255,255,255,0.40)",
    "variable": "var(--colors-chrome\\.40)"
  },
  "colors.chrome.50": {
    "value": "rgba(255,255,255,0.50)",
    "variable": "var(--colors-chrome\\.50)"
  },
  "colors.chrome.60": {
    "value": "rgba(255,255,255,0.60)",
    "variable": "var(--colors-chrome\\.60)"
  },
  "colors.chrome.70": {
    "value": "rgba(255,255,255,0.70)",
    "variable": "var(--colors-chrome\\.70)"
  },
  "colors.chrome.90": {
    "value": "rgba(255,255,255,0.90)",
    "variable": "var(--colors-chrome\\.90)"
  },
  "colors.warm.04": {
    "value": "rgba(127,99,21,0.04)",
    "variable": "var(--colors-warm\\.04)"
  },
  "colors.warm.10": {
    "value": "rgba(127,99,21,0.10)",
    "variable": "var(--colors-warm\\.10)"
  },
  "colors.warm.25": {
    "value": "rgba(127,99,21,0.25)",
    "variable": "var(--colors-warm\\.25)"
  },
  "colors.warm.30": {
    "value": "rgba(127,99,21,0.30)",
    "variable": "var(--colors-warm\\.30)"
  },
  "colors.warm.40": {
    "value": "rgba(127,99,21,0.40)",
    "variable": "var(--colors-warm\\.40)"
  },
  "colors.creamA.30": {
    "value": "rgba(255,240,194,0.30)",
    "variable": "var(--colors-cream-a\\.30)"
  },
  "colors.ivory.30": {
    "value": "rgba(255,250,235,0.30)",
    "variable": "var(--colors-ivory\\.30)"
  },
  "colors.ivory.50": {
    "value": "rgba(255,250,235,0.50)",
    "variable": "var(--colors-ivory\\.50)"
  },
  "colors.scrim.45": {
    "value": "rgba(31,31,31,0.45)",
    "variable": "var(--colors-scrim\\.45)"
  },
  "colors.scrim.50": {
    "value": "rgba(31,31,31,0.50)",
    "variable": "var(--colors-scrim\\.50)"
  },
  "colors.scrim.55": {
    "value": "rgba(31,31,31,0.55)",
    "variable": "var(--colors-scrim\\.55)"
  },
  "colors.scrim.60": {
    "value": "rgba(31,31,31,0.60)",
    "variable": "var(--colors-scrim\\.60)"
  },
  "colors.grid.06": {
    "value": "rgba(128,128,128,0.06)",
    "variable": "var(--colors-grid\\.06)"
  },
  "colors.grid.15": {
    "value": "rgba(128,128,128,0.15)",
    "variable": "var(--colors-grid\\.15)"
  },
  "colors.grid.20": {
    "value": "rgba(128,128,128,0.20)",
    "variable": "var(--colors-grid\\.20)"
  },
  "colors.diff.add.bg": {
    "value": "rgba(46,160,67,0.15)",
    "variable": "var(--colors-diff\\.add\\.bg)"
  },
  "colors.diff.add.emphasis": {
    "value": "rgba(46,160,67,0.2)",
    "variable": "var(--colors-diff\\.add\\.emphasis)"
  },
  "colors.diff.del.bg": {
    "value": "rgba(248,81,73,0.15)",
    "variable": "var(--colors-diff\\.del\\.bg)"
  },
  "colors.diff.del.emphasis": {
    "value": "rgba(248,81,73,0.2)",
    "variable": "var(--colors-diff\\.del\\.emphasis)"
  },
  "colors.slate.05": {
    "value": "rgba(130,130,160,0.05)",
    "variable": "var(--colors-slate\\.05)"
  },
  "colors.slate.08": {
    "value": "rgba(130,130,160,0.08)",
    "variable": "var(--colors-slate\\.08)"
  },
  "colors.slate.10": {
    "value": "rgba(130,130,160,0.10)",
    "variable": "var(--colors-slate\\.10)"
  },
  "colors.slate.15": {
    "value": "rgba(130,130,160,0.15)",
    "variable": "var(--colors-slate\\.15)"
  },
  "fonts.heading": {
    "value": "'Ysabeau Infant', Arial, ui-sans-serif, system-ui, sans-serif",
    "variable": "var(--fonts-heading)"
  },
  "fonts.body": {
    "value": "'Ysabeau Infant', Arial, ui-sans-serif, system-ui, sans-serif",
    "variable": "var(--fonts-body)"
  },
  "fonts.mono": {
    "value": "'Monaspace Argon', 'SF Mono', 'Fira Code', monospace",
    "variable": "var(--fonts-mono)"
  },
  "fontWeights.display": {
    "value": "431",
    "variable": "var(--font-weights-display)"
  },
  "fontWeights.heading": {
    "value": "575",
    "variable": "var(--font-weights-heading)"
  },
  "fontWeights.body": {
    "value": "647",
    "variable": "var(--font-weights-body)"
  },
  "fontWeights.button": {
    "value": "791",
    "variable": "var(--font-weights-button)"
  },
  "shadows.golden": {
    "value": "-3px 5px 13px rgba(127,99,21,0.15), -11px 21px 32px rgba(127,99,21,0.11), -21px 43px 53px rgba(127,99,21,0.08), -43px 85px 80px rgba(127,99,21,0.05)",
    "variable": "var(--shadows-golden)"
  },
  "shadows.goldenDark": {
    "value": "-3px 5px 13px rgba(127,99,21,0.12), -11px 21px 32px rgba(127,99,21,0.09), -21px 43px 53px rgba(127,99,21,0.06), -43px 85px 80px rgba(127,99,21,0.04)",
    "variable": "var(--shadows-golden-dark)"
  },
  "shadows.nav": {
    "value": "0 3px 13px rgba(127,99,21,0.08)",
    "variable": "var(--shadows-nav)"
  },
  "shadows.code": {
    "value": "var(--shadows-code)",
    "variable": "var(--shadows-code)"
  },
  "shadows.focusRing.sm": {
    "value": "0 0 0 2px var(--colors-accent\\.15)",
    "variable": "var(--shadows-focus-ring\\.sm)"
  },
  "shadows.focusRing.md": {
    "value": "0 0 0 3px var(--colors-accent\\.20)",
    "variable": "var(--shadows-focus-ring\\.md)"
  },
  "shadows.focusRing.lg": {
    "value": "0 0 0 2px var(--colors-accent\\.30)",
    "variable": "var(--shadows-focus-ring\\.lg)"
  },
  "shadows.focusRing.xl": {
    "value": "0 0 0 3px var(--colors-accent\\.30)",
    "variable": "var(--shadows-focus-ring\\.xl)"
  },
  "shadows.focusRing.2xl": {
    "value": "0 0 0 2px var(--colors-accent\\.40)",
    "variable": "var(--shadows-focus-ring\\.2xl)"
  },
  "shadows.thumb": {
    "value": "0 1px 3px rgba(0,0,0,0.15)",
    "variable": "var(--shadows-thumb)"
  },
  "shadows.thumbSoft": {
    "value": "0 1px 3px rgba(0,0,0,0.12)",
    "variable": "var(--shadows-thumb-soft)"
  },
  "shadows.drawer": {
    "value": "4px 0 20px rgba(0,0,0,0.15)",
    "variable": "var(--shadows-drawer)"
  },
  "shadows.pop": {
    "value": "0 8px 24px rgba(0,0,0,0.12)",
    "variable": "var(--shadows-pop)"
  },
  "fontSizes.9": {
    "value": "9px",
    "variable": "var(--font-sizes-9)"
  },
  "fontSizes.11": {
    "value": "11px",
    "variable": "var(--font-sizes-11)"
  },
  "fontSizes.13": {
    "value": "13px",
    "variable": "var(--font-sizes-13)"
  },
  "fontSizes.15": {
    "value": "15px",
    "variable": "var(--font-sizes-15)"
  },
  "fontSizes.36": {
    "value": "36px",
    "variable": "var(--font-sizes-36)"
  },
  "fontSizes.40": {
    "value": "40px",
    "variable": "var(--font-sizes-40)"
  },
  "fontSizes.2xs": {
    "value": "0.625rem",
    "variable": "var(--font-sizes-2xs)"
  },
  "fontSizes.xs": {
    "value": "0.75rem",
    "variable": "var(--font-sizes-xs)"
  },
  "fontSizes.sm": {
    "value": "0.875rem",
    "variable": "var(--font-sizes-sm)"
  },
  "fontSizes.md": {
    "value": "1rem",
    "variable": "var(--font-sizes-md)"
  },
  "fontSizes.lg": {
    "value": "1.125rem",
    "variable": "var(--font-sizes-lg)"
  },
  "fontSizes.xl": {
    "value": "1.25rem",
    "variable": "var(--font-sizes-xl)"
  },
  "fontSizes.2xl": {
    "value": "1.5rem",
    "variable": "var(--font-sizes-2xl)"
  },
  "fontSizes.3xl": {
    "value": "2rem",
    "variable": "var(--font-sizes-3xl)"
  },
  "fontSizes.4xl": {
    "value": "3rem",
    "variable": "var(--font-sizes-4xl)"
  },
  "fontSizes.5xl": {
    "value": "3.5rem",
    "variable": "var(--font-sizes-5xl)"
  },
  "fontSizes.6xl": {
    "value": "5.125rem",
    "variable": "var(--font-sizes-6xl)"
  },
  "fontSizes.9.5": {
    "value": "9.5px",
    "variable": "var(--font-sizes-9\\.5)"
  },
  "fontSizes.10.5": {
    "value": "10.5px",
    "variable": "var(--font-sizes-10\\.5)"
  },
  "fontSizes.13.5": {
    "value": "13.5px",
    "variable": "var(--font-sizes-13\\.5)"
  },
  "radii.sm": {
    "value": "2px",
    "variable": "var(--radii-sm)"
  },
  "radii.md": {
    "value": "4px",
    "variable": "var(--radii-md)"
  },
  "radii.lg": {
    "value": "12px",
    "variable": "var(--radii-lg)"
  },
  "radii.full": {
    "value": "9999px",
    "variable": "var(--radii-full)"
  },
  "spacing.0": {
    "value": "0px",
    "variable": "var(--spacing-0)"
  },
  "spacing.1": {
    "value": "4px",
    "variable": "var(--spacing-1)"
  },
  "spacing.2": {
    "value": "8px",
    "variable": "var(--spacing-2)"
  },
  "spacing.3": {
    "value": "12px",
    "variable": "var(--spacing-3)"
  },
  "spacing.4": {
    "value": "16px",
    "variable": "var(--spacing-4)"
  },
  "spacing.5": {
    "value": "20px",
    "variable": "var(--spacing-5)"
  },
  "spacing.6": {
    "value": "24px",
    "variable": "var(--spacing-6)"
  },
  "spacing.7": {
    "value": "28px",
    "variable": "var(--spacing-7)"
  },
  "spacing.8": {
    "value": "32px",
    "variable": "var(--spacing-8)"
  },
  "spacing.9": {
    "value": "36px",
    "variable": "var(--spacing-9)"
  },
  "spacing.10": {
    "value": "40px",
    "variable": "var(--spacing-10)"
  },
  "spacing.12": {
    "value": "48px",
    "variable": "var(--spacing-12)"
  },
  "spacing.14": {
    "value": "56px",
    "variable": "var(--spacing-14)"
  },
  "spacing.15": {
    "value": "60px",
    "variable": "var(--spacing-15)"
  },
  "spacing.16": {
    "value": "64px",
    "variable": "var(--spacing-16)"
  },
  "spacing.20": {
    "value": "80px",
    "variable": "var(--spacing-20)"
  },
  "spacing.24": {
    "value": "96px",
    "variable": "var(--spacing-24)"
  },
  "spacing.25": {
    "value": "100px",
    "variable": "var(--spacing-25)"
  },
  "spacing.28": {
    "value": "112px",
    "variable": "var(--spacing-28)"
  },
  "spacing.30": {
    "value": "120px",
    "variable": "var(--spacing-30)"
  },
  "spacing.32": {
    "value": "128px",
    "variable": "var(--spacing-32)"
  },
  "spacing.36": {
    "value": "144px",
    "variable": "var(--spacing-36)"
  },
  "spacing.40": {
    "value": "160px",
    "variable": "var(--spacing-40)"
  },
  "spacing.44": {
    "value": "176px",
    "variable": "var(--spacing-44)"
  },
  "spacing.45": {
    "value": "180px",
    "variable": "var(--spacing-45)"
  },
  "spacing.48": {
    "value": "192px",
    "variable": "var(--spacing-48)"
  },
  "spacing.50": {
    "value": "200px",
    "variable": "var(--spacing-50)"
  },
  "spacing.52": {
    "value": "208px",
    "variable": "var(--spacing-52)"
  },
  "spacing.56": {
    "value": "224px",
    "variable": "var(--spacing-56)"
  },
  "spacing.60": {
    "value": "240px",
    "variable": "var(--spacing-60)"
  },
  "spacing.64": {
    "value": "256px",
    "variable": "var(--spacing-64)"
  },
  "spacing.65": {
    "value": "260px",
    "variable": "var(--spacing-65)"
  },
  "spacing.72": {
    "value": "288px",
    "variable": "var(--spacing-72)"
  },
  "spacing.74": {
    "value": "296px",
    "variable": "var(--spacing-74)"
  },
  "spacing.75": {
    "value": "300px",
    "variable": "var(--spacing-75)"
  },
  "spacing.80": {
    "value": "320px",
    "variable": "var(--spacing-80)"
  },
  "spacing.96": {
    "value": "384px",
    "variable": "var(--spacing-96)"
  },
  "spacing.0.25": {
    "value": "1px",
    "variable": "var(--spacing-0\\.25)"
  },
  "spacing.0.5": {
    "value": "2px",
    "variable": "var(--spacing-0\\.5)"
  },
  "spacing.0.75": {
    "value": "3px",
    "variable": "var(--spacing-0\\.75)"
  },
  "spacing.1.5": {
    "value": "6px",
    "variable": "var(--spacing-1\\.5)"
  },
  "spacing.2.25": {
    "value": "9px",
    "variable": "var(--spacing-2\\.25)"
  },
  "spacing.2.5": {
    "value": "10px",
    "variable": "var(--spacing-2\\.5)"
  },
  "spacing.3.5": {
    "value": "14px",
    "variable": "var(--spacing-3\\.5)"
  },
  "spacing.4.5": {
    "value": "18px",
    "variable": "var(--spacing-4\\.5)"
  },
  "spacing.5.5": {
    "value": "22px",
    "variable": "var(--spacing-5\\.5)"
  },
  "spacing.7.5": {
    "value": "30px",
    "variable": "var(--spacing-7\\.5)"
  },
  "sizes.0": {
    "value": "0px",
    "variable": "var(--sizes-0)"
  },
  "sizes.1": {
    "value": "4px",
    "variable": "var(--sizes-1)"
  },
  "sizes.2": {
    "value": "8px",
    "variable": "var(--sizes-2)"
  },
  "sizes.3": {
    "value": "12px",
    "variable": "var(--sizes-3)"
  },
  "sizes.4": {
    "value": "16px",
    "variable": "var(--sizes-4)"
  },
  "sizes.5": {
    "value": "20px",
    "variable": "var(--sizes-5)"
  },
  "sizes.6": {
    "value": "24px",
    "variable": "var(--sizes-6)"
  },
  "sizes.7": {
    "value": "28px",
    "variable": "var(--sizes-7)"
  },
  "sizes.8": {
    "value": "32px",
    "variable": "var(--sizes-8)"
  },
  "sizes.9": {
    "value": "36px",
    "variable": "var(--sizes-9)"
  },
  "sizes.10": {
    "value": "40px",
    "variable": "var(--sizes-10)"
  },
  "sizes.12": {
    "value": "48px",
    "variable": "var(--sizes-12)"
  },
  "sizes.14": {
    "value": "56px",
    "variable": "var(--sizes-14)"
  },
  "sizes.15": {
    "value": "60px",
    "variable": "var(--sizes-15)"
  },
  "sizes.16": {
    "value": "64px",
    "variable": "var(--sizes-16)"
  },
  "sizes.20": {
    "value": "80px",
    "variable": "var(--sizes-20)"
  },
  "sizes.24": {
    "value": "96px",
    "variable": "var(--sizes-24)"
  },
  "sizes.25": {
    "value": "100px",
    "variable": "var(--sizes-25)"
  },
  "sizes.28": {
    "value": "112px",
    "variable": "var(--sizes-28)"
  },
  "sizes.30": {
    "value": "120px",
    "variable": "var(--sizes-30)"
  },
  "sizes.32": {
    "value": "128px",
    "variable": "var(--sizes-32)"
  },
  "sizes.36": {
    "value": "144px",
    "variable": "var(--sizes-36)"
  },
  "sizes.40": {
    "value": "160px",
    "variable": "var(--sizes-40)"
  },
  "sizes.44": {
    "value": "176px",
    "variable": "var(--sizes-44)"
  },
  "sizes.45": {
    "value": "180px",
    "variable": "var(--sizes-45)"
  },
  "sizes.48": {
    "value": "192px",
    "variable": "var(--sizes-48)"
  },
  "sizes.50": {
    "value": "200px",
    "variable": "var(--sizes-50)"
  },
  "sizes.52": {
    "value": "208px",
    "variable": "var(--sizes-52)"
  },
  "sizes.56": {
    "value": "224px",
    "variable": "var(--sizes-56)"
  },
  "sizes.60": {
    "value": "240px",
    "variable": "var(--sizes-60)"
  },
  "sizes.64": {
    "value": "256px",
    "variable": "var(--sizes-64)"
  },
  "sizes.65": {
    "value": "260px",
    "variable": "var(--sizes-65)"
  },
  "sizes.72": {
    "value": "288px",
    "variable": "var(--sizes-72)"
  },
  "sizes.74": {
    "value": "296px",
    "variable": "var(--sizes-74)"
  },
  "sizes.75": {
    "value": "300px",
    "variable": "var(--sizes-75)"
  },
  "sizes.80": {
    "value": "320px",
    "variable": "var(--sizes-80)"
  },
  "sizes.90": {
    "value": "360px",
    "variable": "var(--sizes-90)"
  },
  "sizes.95": {
    "value": "380px",
    "variable": "var(--sizes-95)"
  },
  "sizes.96": {
    "value": "384px",
    "variable": "var(--sizes-96)"
  },
  "sizes.100": {
    "value": "400px",
    "variable": "var(--sizes-100)"
  },
  "sizes.120": {
    "value": "480px",
    "variable": "var(--sizes-120)"
  },
  "sizes.125": {
    "value": "500px",
    "variable": "var(--sizes-125)"
  },
  "sizes.140": {
    "value": "560px",
    "variable": "var(--sizes-140)"
  },
  "sizes.150": {
    "value": "600px",
    "variable": "var(--sizes-150)"
  },
  "sizes.160": {
    "value": "640px",
    "variable": "var(--sizes-160)"
  },
  "sizes.180": {
    "value": "720px",
    "variable": "var(--sizes-180)"
  },
  "sizes.220": {
    "value": "880px",
    "variable": "var(--sizes-220)"
  },
  "sizes.225": {
    "value": "900px",
    "variable": "var(--sizes-225)"
  },
  "sizes.300": {
    "value": "1200px",
    "variable": "var(--sizes-300)"
  },
  "sizes.360": {
    "value": "1440px",
    "variable": "var(--sizes-360)"
  },
  "sizes.0.25": {
    "value": "1px",
    "variable": "var(--sizes-0\\.25)"
  },
  "sizes.0.5": {
    "value": "2px",
    "variable": "var(--sizes-0\\.5)"
  },
  "sizes.0.75": {
    "value": "3px",
    "variable": "var(--sizes-0\\.75)"
  },
  "sizes.1.5": {
    "value": "6px",
    "variable": "var(--sizes-1\\.5)"
  },
  "sizes.2.25": {
    "value": "9px",
    "variable": "var(--sizes-2\\.25)"
  },
  "sizes.2.5": {
    "value": "10px",
    "variable": "var(--sizes-2\\.5)"
  },
  "sizes.3.5": {
    "value": "14px",
    "variable": "var(--sizes-3\\.5)"
  },
  "sizes.4.5": {
    "value": "18px",
    "variable": "var(--sizes-4\\.5)"
  },
  "sizes.5.5": {
    "value": "22px",
    "variable": "var(--sizes-5\\.5)"
  },
  "sizes.7.5": {
    "value": "30px",
    "variable": "var(--sizes-7\\.5)"
  },
  "sizes.breakpoint-sm": {
    "value": "640px",
    "variable": "var(--sizes-breakpoint-sm)"
  },
  "sizes.breakpoint-md": {
    "value": "768px",
    "variable": "var(--sizes-breakpoint-md)"
  },
  "sizes.breakpoint-lg": {
    "value": "1024px",
    "variable": "var(--sizes-breakpoint-lg)"
  },
  "sizes.breakpoint-xl": {
    "value": "1280px",
    "variable": "var(--sizes-breakpoint-xl)"
  },
  "lineHeights.none": {
    "value": "1",
    "variable": "var(--line-heights-none)"
  },
  "lineHeights.tight": {
    "value": "1.25",
    "variable": "var(--line-heights-tight)"
  },
  "lineHeights.snug": {
    "value": "1.375",
    "variable": "var(--line-heights-snug)"
  },
  "lineHeights.normal": {
    "value": "1.5",
    "variable": "var(--line-heights-normal)"
  },
  "lineHeights.relaxed": {
    "value": "1.625",
    "variable": "var(--line-heights-relaxed)"
  },
  "lineHeights.loose": {
    "value": "2",
    "variable": "var(--line-heights-loose)"
  },
  "breakpoints.sm": {
    "value": "640px",
    "variable": "var(--breakpoints-sm)"
  },
  "breakpoints.md": {
    "value": "768px",
    "variable": "var(--breakpoints-md)"
  },
  "breakpoints.lg": {
    "value": "1024px",
    "variable": "var(--breakpoints-lg)"
  },
  "breakpoints.xl": {
    "value": "1280px",
    "variable": "var(--breakpoints-xl)"
  },
  "colors.bg.page": {
    "value": "var(--colors-bg\\.page)",
    "variable": "var(--colors-bg\\.page)"
  },
  "colors.bg.card": {
    "value": "var(--colors-bg\\.card)",
    "variable": "var(--colors-bg\\.card)"
  },
  "colors.bg.nav": {
    "value": "var(--colors-bg\\.nav)",
    "variable": "var(--colors-bg\\.nav)"
  },
  "colors.text.primary": {
    "value": "var(--colors-text\\.primary)",
    "variable": "var(--colors-text\\.primary)"
  },
  "colors.text.secondary": {
    "value": "var(--colors-text\\.secondary)",
    "variable": "var(--colors-text\\.secondary)"
  },
  "colors.text.muted": {
    "value": "var(--colors-text\\.muted)",
    "variable": "var(--colors-text\\.muted)"
  },
  "colors.border.default": {
    "value": "var(--colors-border\\.default)",
    "variable": "var(--colors-border\\.default)"
  },
  "colors.border.subtle": {
    "value": "var(--colors-border\\.subtle)",
    "variable": "var(--colors-border\\.subtle)"
  },
  "colors.accent": {
    "value": "var(--colors-accent)",
    "variable": "var(--colors-accent)"
  },
  "colors.sectionLabel": {
    "value": "var(--colors-section-label)",
    "variable": "var(--colors-section-label)"
  },
  "spacing.-0": {
    "value": "calc(var(--spacing-0) * -1)",
    "variable": "var(--spacing-0)"
  },
  "spacing.-1": {
    "value": "calc(var(--spacing-1) * -1)",
    "variable": "var(--spacing-1)"
  },
  "spacing.-2": {
    "value": "calc(var(--spacing-2) * -1)",
    "variable": "var(--spacing-2)"
  },
  "spacing.-3": {
    "value": "calc(var(--spacing-3) * -1)",
    "variable": "var(--spacing-3)"
  },
  "spacing.-4": {
    "value": "calc(var(--spacing-4) * -1)",
    "variable": "var(--spacing-4)"
  },
  "spacing.-5": {
    "value": "calc(var(--spacing-5) * -1)",
    "variable": "var(--spacing-5)"
  },
  "spacing.-6": {
    "value": "calc(var(--spacing-6) * -1)",
    "variable": "var(--spacing-6)"
  },
  "spacing.-7": {
    "value": "calc(var(--spacing-7) * -1)",
    "variable": "var(--spacing-7)"
  },
  "spacing.-8": {
    "value": "calc(var(--spacing-8) * -1)",
    "variable": "var(--spacing-8)"
  },
  "spacing.-9": {
    "value": "calc(var(--spacing-9) * -1)",
    "variable": "var(--spacing-9)"
  },
  "spacing.-10": {
    "value": "calc(var(--spacing-10) * -1)",
    "variable": "var(--spacing-10)"
  },
  "spacing.-12": {
    "value": "calc(var(--spacing-12) * -1)",
    "variable": "var(--spacing-12)"
  },
  "spacing.-14": {
    "value": "calc(var(--spacing-14) * -1)",
    "variable": "var(--spacing-14)"
  },
  "spacing.-15": {
    "value": "calc(var(--spacing-15) * -1)",
    "variable": "var(--spacing-15)"
  },
  "spacing.-16": {
    "value": "calc(var(--spacing-16) * -1)",
    "variable": "var(--spacing-16)"
  },
  "spacing.-20": {
    "value": "calc(var(--spacing-20) * -1)",
    "variable": "var(--spacing-20)"
  },
  "spacing.-24": {
    "value": "calc(var(--spacing-24) * -1)",
    "variable": "var(--spacing-24)"
  },
  "spacing.-25": {
    "value": "calc(var(--spacing-25) * -1)",
    "variable": "var(--spacing-25)"
  },
  "spacing.-28": {
    "value": "calc(var(--spacing-28) * -1)",
    "variable": "var(--spacing-28)"
  },
  "spacing.-30": {
    "value": "calc(var(--spacing-30) * -1)",
    "variable": "var(--spacing-30)"
  },
  "spacing.-32": {
    "value": "calc(var(--spacing-32) * -1)",
    "variable": "var(--spacing-32)"
  },
  "spacing.-36": {
    "value": "calc(var(--spacing-36) * -1)",
    "variable": "var(--spacing-36)"
  },
  "spacing.-40": {
    "value": "calc(var(--spacing-40) * -1)",
    "variable": "var(--spacing-40)"
  },
  "spacing.-44": {
    "value": "calc(var(--spacing-44) * -1)",
    "variable": "var(--spacing-44)"
  },
  "spacing.-45": {
    "value": "calc(var(--spacing-45) * -1)",
    "variable": "var(--spacing-45)"
  },
  "spacing.-48": {
    "value": "calc(var(--spacing-48) * -1)",
    "variable": "var(--spacing-48)"
  },
  "spacing.-50": {
    "value": "calc(var(--spacing-50) * -1)",
    "variable": "var(--spacing-50)"
  },
  "spacing.-52": {
    "value": "calc(var(--spacing-52) * -1)",
    "variable": "var(--spacing-52)"
  },
  "spacing.-56": {
    "value": "calc(var(--spacing-56) * -1)",
    "variable": "var(--spacing-56)"
  },
  "spacing.-60": {
    "value": "calc(var(--spacing-60) * -1)",
    "variable": "var(--spacing-60)"
  },
  "spacing.-64": {
    "value": "calc(var(--spacing-64) * -1)",
    "variable": "var(--spacing-64)"
  },
  "spacing.-65": {
    "value": "calc(var(--spacing-65) * -1)",
    "variable": "var(--spacing-65)"
  },
  "spacing.-72": {
    "value": "calc(var(--spacing-72) * -1)",
    "variable": "var(--spacing-72)"
  },
  "spacing.-74": {
    "value": "calc(var(--spacing-74) * -1)",
    "variable": "var(--spacing-74)"
  },
  "spacing.-75": {
    "value": "calc(var(--spacing-75) * -1)",
    "variable": "var(--spacing-75)"
  },
  "spacing.-80": {
    "value": "calc(var(--spacing-80) * -1)",
    "variable": "var(--spacing-80)"
  },
  "spacing.-96": {
    "value": "calc(var(--spacing-96) * -1)",
    "variable": "var(--spacing-96)"
  },
  "spacing.-0.25": {
    "value": "calc(var(--spacing-0\\.25) * -1)",
    "variable": "var(--spacing-0\\.25)"
  },
  "spacing.-0.5": {
    "value": "calc(var(--spacing-0\\.5) * -1)",
    "variable": "var(--spacing-0\\.5)"
  },
  "spacing.-0.75": {
    "value": "calc(var(--spacing-0\\.75) * -1)",
    "variable": "var(--spacing-0\\.75)"
  },
  "spacing.-1.5": {
    "value": "calc(var(--spacing-1\\.5) * -1)",
    "variable": "var(--spacing-1\\.5)"
  },
  "spacing.-2.25": {
    "value": "calc(var(--spacing-2\\.25) * -1)",
    "variable": "var(--spacing-2\\.25)"
  },
  "spacing.-2.5": {
    "value": "calc(var(--spacing-2\\.5) * -1)",
    "variable": "var(--spacing-2\\.5)"
  },
  "spacing.-3.5": {
    "value": "calc(var(--spacing-3\\.5) * -1)",
    "variable": "var(--spacing-3\\.5)"
  },
  "spacing.-4.5": {
    "value": "calc(var(--spacing-4\\.5) * -1)",
    "variable": "var(--spacing-4\\.5)"
  },
  "spacing.-5.5": {
    "value": "calc(var(--spacing-5\\.5) * -1)",
    "variable": "var(--spacing-5\\.5)"
  },
  "spacing.-7.5": {
    "value": "calc(var(--spacing-7\\.5) * -1)",
    "variable": "var(--spacing-7\\.5)"
  },
  "colors.colorPalette": {
    "value": "var(--colors-color-palette)",
    "variable": "var(--colors-color-palette)"
  }
}

export function token(path, fallback) {
  return tokens[path]?.value || fallback
}

function tokenVar(path, fallback) {
  return tokens[path]?.variable || fallback
}

token.var = tokenVar