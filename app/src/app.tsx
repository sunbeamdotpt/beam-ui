...
import { Link as RouterLink, Outlet, Route, Routes, useLocation, useNavigate } from "react-router-dom";
...
/** Adapter that turns react-router's `to` prop into the `href` shape the design system expects. */
...
function DocsLayoutWrapper() {
  const { pathname } = useLocation();
  return (
    <DocsLayout pageDates={pageDates} currentPath={pathname} linkAs={Link}>
      <Outlet />
    </DocsLayout>
  );
}
...
function ApiLayoutWrapper() {
  const { pathname } = useLocation();
  return (
    <ApiLayout currentPath={pathname} linkAs={Link}>
      <Outlet />
    </ApiLayout>
  );
}

function FullwidthLayoutWrapper() {
  const { pathname } = useLocation();
  return (
    <FullwidthLayout currentPath={pathname} linkAs={Link}>
      <Outlet />
    </FullwidthLayout>
  );
}
    ...
    <>
      ...
      <Routes>
        ...
        <Route element={<RouterShell />}>
          ...
          <Route element={<DocsLayoutWrapper />}>
            {/* Foundations */}
          ...
          <Route element={<FullwidthLayoutWrapper />}>
          ...
          <Route element={<ApiLayoutWrapper />}>
